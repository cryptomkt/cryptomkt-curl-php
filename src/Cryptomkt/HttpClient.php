...
use Cryptomkt\Authentication\Authentication;
use Cryptomkt\Enum\Param;
...
class HttpClient
{
    ...
    private function send($method, $path, $params = [])
    {
        ...
        //
        $this->transport->setHeaders($options['headers']);

        try {
            // $this->lastResponse = $response = $this->transport->sendRequest($request);
            switch ($method) {
                case 'GET':
                    $this->lastResponse = $response = $this->transport->get($this->prepareUrl($path));
                    break;
                case 'POST':
                    $this->lastResponse = $response = $this->transport->post($this->prepareUrl($path), $options['form_params']);
                    break;
            }
